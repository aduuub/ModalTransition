...
/// An interaction controller that handles the swipe to dismiss
class StandardInteractionController: NSObject {
    
    var interactionInProgress = false
    
    private weak var viewController: ModalTransitionPresentable?
    ...
    init(viewController: ModalTransitionPresentable) {
    ...
    @objc
    func handleGesture(_ gestureRecognizer: OneWayPanGestureRecognizer) {
...
extension StandardInteractionController: InteractionControlling {
    ...
    func update(progress: CGFloat) {
        ...
        if let modalPresentationController = presentedViewController.presentationController as? ModalPresentationController {
            modalPresentationController.setFadeViewAlpha(1.0 - progress)
        }
    }
    ...
    func cancel(initialSpringVelocity: CGFloat) {
        ...
        cancellationAnimator?.addAnimations {
            ...
            if let modalPresentationController = presentedViewController.presentationController as? ModalPresentationController {
                modalPresentationController.setFadeViewAlpha(1)
            }
        }
    ...
    func finish(initialSpringVelocity: CGFloat) {
        guard let transitionContext = transitionContext, let presentedFrame = presentedFrame else { return }
        let presentedViewController = transitionContext.viewController(forKey: .from) as! ModalTransitionPresentable
        ...
        finishAnimator.addAnimations {
            ...
            if let modalPresentationController = presentedViewController.presentationController as? ModalPresentationController {
                modalPresentationController.setFadeViewAlpha(0)
            }
        }
...
extension StandardInteractionController {
